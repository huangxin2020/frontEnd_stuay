...
//jq中的each和内置的FOR-EACH类似（但是FOR-EACH只能遍历数组），用来遍历数组（类数组、对象）中的每一项的
// 使用jq需要记得把jq引进来
$.each();
...
// 基于jq的each方法，自己封装一个可以遍历数组、类数组、对象的方法
...
String.prototype.myReplace = function myReplace(reg, callback=new Function()) {
    //=>this:str
    //=>默认REG肯定加G了,CALLBACK肯定传递函数了
    let res = reg.exec(this),
// 匹配返回的结果是['{0}','0',index:11,input:"my name is {0}，i am {1} years old!"];
        _this = this;
    while (res) {
        //=>res:每一次EXEC捕获的结果（数组）
